...
app.post("/api/auth/signup", async (req, res) => {
  ...
  if (!user_name || !email || !password || !first_name || !role) {
    return res
      .status(400)
      .json({
        message: "Please provide all required fields, including a role.",
      });
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Server error during customer creation." });
    } finally {
      client.release();
    }
  }
...
app.get("/api/ornaments", async (req, res) => {
  try {
    const result = await db.query(
      "SELECT ornament_id, ornament_type, ornament_name, description FROM datamanagement.ornament_details ORDER BY created_on DESC"
    );
...
app.post("/api/ornaments", async (req, res) => {
  ...
  if (!ornament_type || !ornament_name) {
    return res
      .status(400)
      .json({ message: "Ornament type and name are required." });
  }
...
app.post("/api/karats", async (req, res) => {
  ...
  if (!karat_name || loan_percentage === undefined) {
    return res
      .status(400)
      .json({ message: "Karat name and loan percentage are required." });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Gold karat detail created successfully!" });
  } catch (error) {
...
app.post("/api/gold-rates", async (req, res) => {
  ...
  try {
    ...
    if (karatDetails.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Selected Karat does not exist." });
    }
