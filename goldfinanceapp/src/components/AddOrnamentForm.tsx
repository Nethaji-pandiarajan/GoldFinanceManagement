...
export default function AddOrnamentForm({ mode, initialData, onClose, onSuccess, setAlert }: AddOrnamentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'edit') {
        await axios.put(`${API_BASE_URL}/api/ornaments/${initialData.ornament_id}`, formData);
      } else {
        await axios.post(`${API_BASE_URL}/api/ornaments`, formData);
      }
      onSuccess();
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || `Failed to ${mode} ornament.`;
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="relative bg-[#111315] rounded-lg shadow-xl p-8 w-full max-w-lg">
        ...
        <h2 className="text-2xl font-bold text-[#c69909] mb-6">{mode === 'edit' ? 'Edit Ornament' : 'Add New Ornament'}</h2>
        
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div>
              <label className={labelStyle}>Ornament Type*</label>
              <input type="text" name="ornament_type" value={formData.ornament_type} onChange={handleChange} className={inputStyle} required placeholder="e.g., Ring, Necklace, Bangle" />
            </div>
            <div>
              <label className={labelStyle}>Ornament Name*</label>
              <input type="text" name="ornament_name" value={formData.ornament_name} onChange={handleChange} className={inputStyle} required placeholder="e.g., Gold Wedding Ring" />
            </div>
            <div>
          ...
          <div className="flex justify-end space-x-4 mt-8">
            ...
            <button type="submit" className="px-6 py-2 rounded bg-[#c69909] hover:bg-yellow-500 text-black font-semibold" disabled={loading}>
              {loading ? 'Saving...' : (mode === 'edit' ? 'Update Ornament' : 'Save Ornament')}
            </button>
          </div>
        </form>
      </div>
