...
export default function AddOrnamentForm({
  mode,
  initialData,
  onClose,
  onSuccess,
  setAlert,
}: AddOrnamentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || `Failed to ${mode} ornament.`;
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="relative bg-[#111315] rounded-lg shadow-xl p-8 w-full max-w-lg">
        ...
        <h2 className="text-2xl font-bold text-[#c69909] mb-6">
          {mode === "edit" ? "Edit Ornament" : "Add New Ornament"}
        </h2>
