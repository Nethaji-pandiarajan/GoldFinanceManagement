...
export default function AmountModal({ user, action, onClose, onSuccess, setAlert }: AmountModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(numericAmount) || numericAmount <= 0) {
      setAlert({ show: true, type: 'error', message: 'Please enter a valid, positive amount.' });
      return;
    }
    if (!isAddAction && numericAmount > parseFloat(user.total_invested)) {
      setAlert({ show: true, type: 'error', message: 'Cannot remove more than the total invested amount.' });
      return;
    }
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || `Failed to ${action} amount.`;
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div>
              ...
              {!isAddAction && (
                <p className="text-xs text-gray-500 mt-1">
                  Total Invested: ₹{parseFloat(user.total_invested).toLocaleString()}
                </p>
              )}
            </div>
          </div>
          <div className="flex justify-end space-x-4 mt-8">
            ...
            <button type="submit" className={clsx(
              ...
              isAddAction ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
            )} disabled={loading}>
              {loading ? 'Processing...' : (isAddAction ? 'Confirm Add' : 'Confirm Remove')}
            </button>
          </div>
        </form>
      </div>
