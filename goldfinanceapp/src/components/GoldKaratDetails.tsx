...
//import ViewKaratModal from './ViewKaratModal';
...
export default function GoldKaratDetails() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setAlert({ show: true, type: 'success', message: 'Karat detail deleted successfully!' });
    } catch (error) {
      setAlert({ show: true, type: 'error', message: 'Could not delete karat detail.' });
    } finally {
      ...
      render: (data: any, type: any, row: any) => {
        ...
        return `
          <div class="flex items-center justify-center space-x-4">
            <button title="Edit Karat" data-action="edit" data-row='${JSON.stringify(row)}' class="p-2 rounded-full text-blue-400 hover:bg-blue-500/20 hover:text-blue-200">${editIcon}</button>
            <button title="Delete Karat" data-action="delete" data-row='${JSON.stringify(row)}' class="p-2 rounded-full text-red-500 hover:bg-red-500/20 hover:text-red-300">${deleteIcon}</button>
          </div>
        `;
      },
    },
  ...
  const handleSuccess = (mode: 'add' | 'edit') => {
    ...
    const message = mode === 'add' ? 'Karat detail created successfully!' : 'Karat detail updated successfully!';
  ...
  return (
    <>
      ...
      {isAddModalOpen && <AddGoldKaratForm mode="add" onClose={() => setAddModalOpen(false)} onSuccess={() => handleSuccess('add')} setAlert={setAlert} />}
      {editData && <AddGoldKaratForm mode="edit" initialData={editData} onClose={() => setEditData(null)} onSuccess={() => handleSuccess('edit')} setAlert={setAlert} />}
      {/* {viewData && <ViewKaratModal karat={viewData} onClose={() => setViewData(null)} />} */}
      {itemToDelete && (
        <ConfirmationDialog type="delete" message="Are you sure you want to delete this karat detail?" onConfirm={handleConfirmDelete} onCancel={() => setItemToDelete(null)} />
      )}
      <div className="bg-[#111315] p-6 rounded-lg shadow-lg">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold text-[#c69909]">Gold Karat Details</h1>
