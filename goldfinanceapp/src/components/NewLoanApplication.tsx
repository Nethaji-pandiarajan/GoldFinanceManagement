...
export default function NewLoanApplication() {
    ...
    const fetchDropdownData = async () => {
      try {
        const [custRes, ornRes, karRes, nomRes] = await Promise.all([
          ...
          api.get(`/api/karats/list`),
          api.get(`/api/nominees-list`),
        ]);
  ...
  const handleCustomerSelect = (customer: any | null) => {
    ...
    if (customer) {
      const relatedNominee = allNominees.find(
        (n) => n.id === customer.nominee_id
      );
      setSelectedNominee(relatedNominee || null);
      setLoanDetails((prev) => ({
        ...prev,
        customer_id: customer.id,
        nominee_id: relatedNominee?.id || "",
      }));
    } else {
  ...
  const validateForm = (): boolean => {
    ...
    for (const field of requiredLoanFields) {
      if (!loanDetails[field as keyof LoanDetails]) {
        ...
        setAlert({
          show: true,
          type: "error",
          message: `Please fill out the "${fieldName}" field.`,
        });
        return false;
      }
    }
    if (
      ...
      parseFloat(loanDetails.eligible_amount)
    ) {
      setAlert({
        show: true,
        type: "error",
        message: "Amount Issued cannot be greater than the Eligible Amount.",
      });
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    const finalLoanDetails = {
      ...loanDetails,
      loan_datetime: `${loanDetails.loan_date}T${
        loanDetails.loan_time || "00:00"
      }`,
    };
    ...
    try {
      await api.post(`/api/loans`, submissionData);
      setAlert({
        show: true,
        type: "success",
      ...
      <div className="bg-[#111315] p-6 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleOpenConfirmModal} className="space-y-8">
            ...
            <div className="relative border border-gray-700/50 rounded-lg p-4">
              ...
              <div className="space-y-4 pt-8">
                {ornamentRows.map((ornament, index) => (
                  <div
                    key={ornament.key}
                    className="relative p-4 border border-gray-700/30 bg-black/20 rounded-lg"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1.5fr_2.5fr_1fr_1fr_2fr_auto] gap-x-4 gap-y-4 items-center">
                      <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="h-12 w-full text-sm text-gray-400 file:mr-2 file:py-2 file:px-3 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#c69909]/80 file:text-black hover:file:bg-yellow-500"
                        />
                      </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label className={labelStyle}>Loan Date*</label>
                <input
                  type="date"
                  name="loan_date"
                  value={loanDetails.loan_date}
              ...
              <div>
                <label className={labelStyle}>Loan Time*</label>
                <input
                  type="time"
                  name="loan_time"
                  value={loanDetails.loan_time}
                  onChange={handleLoanChange}
                  className={inputStyle}
                  required
                />
          ...
          <div className="flex justify-end mt-8">
            <button
              type="submit"
              className="flex items-center justify-center bg-[#c69909] text-black font-bold py-3 px-6 rounded-lg hover:bg-yellow-500 transition-colors text-base"
              disabled={loading}
            >
