  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/users/processed-amounts`
      );
          ...
          <table className="w-full text-left table-fixed">
            <thead className="border-b-2 border-[#c69909]">
              <tr>
                ...
                <th className={`${cellStyle} w-[25%] text-white`}>
                  Total Invested
                </th>
                <th className={`${cellStyle} w-[15%] text-white text-center`}>
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {users.map((user) => (
                <tr key={user.user_id} className="border-b border-gray-800">
                  <td className={`${cellStyle} text-gray-300 truncate`}>
                    {user.user_name}
                  </td>
                  <td className={`${cellStyle} text-gray-300 truncate`}>
                    {user.email}
