...
export default function Schemes() {
    ...
    const handleActionClick = (action: "view" | "edit" | "delete", schemeId: number) => {
        if (action === "view") {
    ...
    const handleSuccess = (mode: "add" | "edit") => {
        ...
        const message = mode === "add" ? "Scheme created successfully!" : "Scheme updated successfully!";
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            setAlert({ show: true, type: "success", message: "Scheme deleted successfully!" });
        } catch (error) {
            setAlert({ show: true, type: "error", message: "Could not delete scheme." });
        } finally {
    ...
    return (
        <>
            ...
            {isAddModalOpen && <AddSchemeForm onClose={() => setAddModalOpen(false)} onSuccess={() => handleSuccess('add')} setAlert={setAlert} />}
            {editSchemeId && <EditSchemeForm schemeId={editSchemeId} onClose={() => setEditSchemeId(null)} onSuccess={() => handleSuccess('edit')} setAlert={setAlert} />}
            {schemeToDelete && <ConfirmationDialog type="delete" message="Are you sure you want to delete this scheme and all its interest slabs?" onConfirm={handleConfirmDelete} onCancel={() => setSchemeToDelete(null)} />}
            {viewSchemeId && <ViewSchemeModal schemeId={viewSchemeId} onClose={() => setViewSchemeId(null)} setAlert={setAlert} />}
            <div className="bg-[#111315] p-6 rounded-lg shadow-lg">
                <div className="flex justify-between items-center mb-4">
                    <h1 className="text-2xl font-bold text-[#c69909]">Scheme Management</h1>
                ...
                <DataTable
                    ...
                    ajax={{ url: `${API_BASE_URL}/api/schemes`, dataSrc: "", headers: {'x-auth-token': localStorage.getItem('authToken') || ''} }}
