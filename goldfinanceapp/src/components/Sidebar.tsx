...
import {
  Bars3Icon, XMarkIcon, DocumentPlusIcon, UserGroupIcon, BuildingStorefrontIcon, ScaleIcon,
  CurrencyDollarIcon, CogIcon ,Squares2X2Icon
...
const navigation = originalNavigation.filter(
  item => item.name !== 'Reports' && item.name !== 'My Account'
);
type SidebarProps = {
  activeItem: string;
...
export default function Sidebar({ activeItem, setActiveItem ,userRole }: SidebarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-center mb-6">
        ...
        >
          {isExpanded ? <XMarkIcon className="h-6 w-6 " /> : <Bars3Icon className="h-6 w-6" />}
        </button>
      </div>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                {
                  'bg-[#c69909] hover:text-black text-white font-semibold': isActive,
