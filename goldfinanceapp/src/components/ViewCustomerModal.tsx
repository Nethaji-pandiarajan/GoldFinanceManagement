...
export default function ViewCustomerModal({ customer, onClose }: { customer: any; onClose: () => void }) {
  const customerImageUrl = bufferToBase64(customer.customer_image);
  const proofImageUrl = bufferToBase64(customer.proof_image);
  const formattedDob = customer.date_of_birth ? new Date(customer.date_of_birth).toLocaleDateString() : 'N/A';
  return(
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-[#111315] rounded-lg shadow-xl p-8 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start">
            <h2 className="text-2xl font-bold text-[#c69909] mb-6">Customer & Nominee Details</h2>
        ...
        <section className="mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <div>
                    ...
                    {customerImageUrl ? 
                      <img src={customerImageUrl} alt="Customer" className="w-full h-auto max-h-80 rounded-lg object-contain bg-black/20"/> : 
                ...
                <div>
                    ...
                    {proofImageUrl ? 
                      <img src={proofImageUrl} alt="Proof" className="w-full h-auto max-h-80 rounded-lg object-contain bg-black/20"/> : 
                      <p className="text-gray-500">No image provided.</p>
                    }
                </div>
            </div>
